...
export const useBreadcrumbs = (): BreadcrumbItem[] => {
  ...
  const { detailName } = useDetailNameContext();
  ...
  return useMemo(() => {
    ...
    if (path.startsWith('/filme/')) {
      const movieTitle =
        detailName || searchParams.get('title') || `Filme ${params.id}`;
      breadcrumbs.push(
    ...
    if (path.startsWith('/serie/')) {
      const showTitle =
        detailName || searchParams.get('title') || `Série ${params.id}`;
      breadcrumbs.push(
    ...
    if (path.startsWith('/pessoa/')) {
      const personName =
        detailName || searchParams.get('name') || `Pessoa ${params.id}`;
      breadcrumbs.push(
    ...
    return breadcrumbs;
  }, [location.pathname, location.search, params, detailName]);
};
