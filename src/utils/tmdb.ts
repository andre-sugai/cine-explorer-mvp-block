 ...
 * @returns Detalhes da coleção
 */
export const getCollectionDetails = async (id: number): Promise<TMDBCollection> => {
  try {
...
/**
 * Busca coleções populares usando estratégia de buscar filmes populares 
 * que fazem parte de coleções e depois extrair as coleções únicas.
 * @param maxCollections Número máximo de coleções a retornar
 * @returns Array de coleções populares
 */
export const getPopularCollections = async (maxCollections: number = 12): Promise<TMDBCollection[]> => {
  try {
    // Buscar filmes populares das primeiras 3 páginas para encontrar mais coleções
    ...
    // Buscar detalhes de filmes que podem ter coleções
    const movieDetailsPromises = allPopularMovies.slice(0, 40).map(movie => 
      getMovieDetails(movie.id).catch(() => null)
    );
    
    const movieDetails = await Promise.all(movieDetailsPromises);
    
    // Filtrar filmes que fazem parte de coleções
    const moviesWithCollections = movieDetails.filter(movie => 
      movie && movie.belongs_to_collection?.id
    );
    
    // Extrair IDs únicos de coleções, priorizando por popularidade do filme
    const collectionIds = new Set<number>();
    const collectionPopularity = new Map<number, number>();
    ...
    moviesWithCollections.forEach(movie => {
      if (movie?.belongs_to_collection?.id) {
        ...
        if (!collectionIds.has(collectionId)) {
          ...
          collectionPopularity.set(collectionId, movie.popularity || 0);
        } else {
          // Se já existe, manter a maior popularidade
          const currentPop = collectionPopularity.get(collectionId) || 0;
          if ((movie.popularity || 0) > currentPop) {
            collectionPopularity.set(collectionId, movie.popularity || 0);
          }
        }
      }
    ...
    // Ordenar por popularidade e pegar apenas as mais populares
    const sortedCollectionIds = Array.from(collectionIds)
      .sort((a, b) => (collectionPopularity.get(b) || 0) - (collectionPopularity.get(a) || 0))
      .slice(0, maxCollections);
    
    // Buscar detalhes de cada coleção
    const collectionsPromises = sortedCollectionIds.map(id => 
      getCollectionDetails(id).catch(err => {
    ...
    // Filtrar resultados nulos e retornar
    return collections.filter((collection): collection is TMDBCollection => 
      collection !== null && collection.parts && collection.parts.length > 1
    );
