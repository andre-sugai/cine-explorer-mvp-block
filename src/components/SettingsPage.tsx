...
export const SettingsPage: React.FC = () => {
  ...
  const { exportData, importData, validateBackupFile, isImporting, isExporting } = useDataManager();
  const { favorites, clearAllFavorites, getStats: getFavoritesStats } = useFavorites();
  const { watched, clearAllWatched, getStats: getWatchedStats } = useWatched();
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="bg-gradient-cinema border-primary/20">
          <CardContent className="p-6 text-center">
            <div className="text-3xl font-bold text-primary mb-2">{favoritesStats.total}</div>
        ...
        <Card className="bg-gradient-cinema border-primary/20">
          <CardContent className="p-6 text-center">
            <div className="text-3xl font-bold text-primary mb-2">{watchedStats.total}</div>
        ...
        <Card className="bg-gradient-cinema border-primary/20">
          <CardContent className="p-6 text-center">
            <div className="text-3xl font-bold text-primary mb-2">{watchedStats.totalHours}h</div>
          ...
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-muted-foreground">
                {currentApiKey ? 'Chave configurada' : 'Nenhuma chave configurada'}
              </p>
              {currentApiKey && (
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Button 
              ...
              disabled={isExporting || (favorites.length === 0 && watched.length === 0)}
          ...
          <p className="text-sm text-muted-foreground">
            Faça backup dos seus favoritos, lista de assistidos e configurações em um arquivo JSON.
          </p>
        </CardContent>
      </Card>
      ...
      <Card className="bg-gradient-cinema border-red-500/20">
        ...
        <CardContent className="space-y-4">
          ...
          <p className="text-sm text-muted-foreground">
            Remove favoritos, lista de assistidos ou todos os dados do aplicativo.
          </p>
        </CardContent>
      </Card>
      ...
      <Dialog open={showApiKeyDialog} onOpenChange={setShowApiKeyDialog}>
        <DialogContent className="bg-gradient-cinema border-primary/20">
          <DialogHeader>
            <DialogTitle className="text-primary">Configurar Chave da API</DialogTitle>
            <DialogDescription>
              Insira sua chave da API do TMDB para acessar os dados dos filmes e séries.
            </DialogDescription>
          </DialogHeader>
      ...
      <Dialog open={showClearDialog} onOpenChange={setShowClearDialog}>
        <DialogContent className="bg-gradient-cinema border-red-500/20">
          ...
          <div className="space-y-4">
            <RadioGroup value={clearType} onValueChange={(value) => setClearType(value as typeof clearType)}>
              <div className="flex items-center space-x-2">
                <RadioGroupItem value="favorites" id="favorites" />
                <Label htmlFor="favorites">Apenas favoritos ({favoritesStats.total} itens)</Label>
              </div>
              <div className="flex items-center space-x-2">
                <RadioGroupItem value="watched" id="watched" />
                <Label htmlFor="watched">Apenas lista de assistidos ({watchedStats.total} itens)</Label>
              </div>
              <div className="flex items-center space-x-2">
                <RadioGroupItem value="all" id="all" />
                <Label htmlFor="all">Todos os dados (favoritos, assistidos e configurações)</Label>
              </div>
            </RadioGroup>
